...
var JavaClassFile = (function (_JavaFile) {
  ...
  function JavaClassFile(javaPoly, script) {
    _classCallCheck(this, JavaClassFile);

    var _this = _possibleConstructorReturn(this, Object.getPrototypeOf(JavaClassFile).call(this, javaPoly, script));
...
var JavaFile = function JavaFile(javaPoly, script) {
  _classCallCheck(this, JavaFile);
...
var JAVA_MIME = [{ // for compiled Java-class
  type: 'class',
    ...
    if (options.initOnStart === true) {
      global.document.addEventListener('DOMContentLoaded', function (e) {
        _.each(global.document.scripts, function (script) {
          var scriptTypes = JAVA_MIME.filter(function (item) {
            return item.mime.some(function (m) {
              return m === script.type;
            });
          });

          // create only when scriptTypes is only 1
          if (scriptTypes.length === 1) {
            var scriptType = scriptTypes[0].type;
            if (scriptTypes[0].srcRequired && !script.src) throw 'An attribute \'src\' should be declared for MIME-type \'' + script.type + '\'';
...
var JavaSourceFile = (function (_JavaFile) {
  _inherits(JavaSourceFile, _JavaFile);

  function JavaSourceFile(javaPoly, script) {
    _classCallCheck(this, JavaSourceFile);

    return _possibleConstructorReturn(this, Object.getPrototypeOf(JavaSourceFile).call(this, javaPoly, script));
  }
...
var _JavaPoly = require('./JavaPoly');
